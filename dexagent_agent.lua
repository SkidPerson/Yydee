...
local function main()
	...
	ds.Init = function()
		...
		-- FRAME RAIZ (RGB 52,52,52 igual ao Frame_2 original)
		local rootFrame = Instance.new("Frame", content)
		rootFrame.BorderSizePixel  = 0
		rootFrame.BackgroundColor3 = Color3.fromRGB(52, 52, 52)
		rootFrame.Size             = UDim2.new(1, 0, 1, 0)
		rootFrame.ClipsDescendants = true

		-- SCROLLINGFRAME (RGB 46,46,46 igual ao ScrollingFrame_3 original)
		local scrollFrame = Instance.new("ScrollingFrame", rootFrame)
		scrollFrame.BorderSizePixel      = 0
		scrollFrame.BackgroundColor3     = Color3.fromRGB(46, 46, 46)
		scrollFrame.Size                 = UDim2.new(1, 0, 0, 656)
		scrollFrame.Position             = UDim2.new(0, 0, 0, 0)
		scrollFrame.CanvasSize           = UDim2.new(0, 0, 0, 0)
		scrollFrame.ScrollBarThickness   = 4
		scrollFrame.ScrollBarImageColor3 = Color3.fromRGB(100, 100, 100)

		-- IMAGEBUTTON toggle (rbxassetid://110844258094575, 12x12, pos 2,4)
		local imageButton = Instance.new("ImageButton", scrollFrame)
		imageButton.BorderSizePixel        = 0
		imageButton.BackgroundColor3       = Color3.fromRGB(255, 255, 255)
		imageButton.BackgroundTransparency = 1
		imageButton.Image                  = "rbxassetid://110844258094575"
		imageButton.Size                   = UDim2.new(0, 12, 0, 12)
		imageButton.Position               = UDim2.new(0, 2, 0, 4)

		-- TEXTLABEL "PlayerData" (TextSize 16, SourceSansPro, RGB 222,222,222, pos 20,6)
		local labelPlayerData = Instance.new("TextLabel", scrollFrame)
		labelPlayerData.BorderSizePixel        = 0
		labelPlayerData.TextSize               = 16
		labelPlayerData.BackgroundColor3       = Color3.fromRGB(255, 255, 255)
		labelPlayerData.BackgroundTransparency = 1
		labelPlayerData.Font                   = Enum.Font.SourceSans
		labelPlayerData.TextColor3             = Color3.fromRGB(222, 222, 222)
		labelPlayerData.Size                   = UDim2.new(0, 138, 0, 10)
		labelPlayerData.Text                   = "PlayerData"
		labelPlayerData.Position               = UDim2.new(0, 20, 0, 6)
		labelPlayerData.TextXAlignment         = Enum.TextXAlignment.Left
		...
		-- TEXTBUTTON de player (TextSize 16, SourceSansPro, RGB 211,211,211, tamanho 172x18, pos X=78)
		local function criarBotaoPlayer(nome, yPos)
			...
			btn.Position               = UDim2.new(0, 78, 0, yPos)
			btn.Text                   = nome
			btn.Visible                = false
		...
		local function reconstruirLista()
			...
			-- LocalPlayer primeiro (index 0 -> yPos 24, igual ao TextButton_6 original)
			local btnLocal = criarBotaoPlayer(
				LocalPlayer and LocalPlayer.Name or "Unknown",
				24 + (index * 18)
			)
			btnLocal.Visible = isListVisible
			table.insert(playerButtons, btnLocal)
			index = index + 1

			for _, plr in ipairs(Players:GetPlayers()) do
				if plr ~= LocalPlayer then
					local b = criarBotaoPlayer(plr.Name, 24 + (index * 18))
					b.Visible = isListVisible
					table.insert(playerButtons, b)
					index = index + 1
				end
			end

			local totalH = 24 + (index * 18) + 10
			scrollFrame.CanvasSize = UDim2.new(0, 0, 0, math.max(totalH, 30))

			if isListVisible then
				local h = math.min(totalH, 400)
				window:Resize(246, h)
			else
				window:Resize(246, 24)
			end
		...
		-- TOGGLE imageButton (igual ao original)
		imageButton.MouseButton1Click:Connect(function()
			...
			if isListVisible then
				...
				local h = math.min(24 + (count * 18) + 10, 400)
				window:Resize(246, h)
			else
				window:Resize(246, 24)
			end
		...
		-- Label tambem togglavel
		labelPlayerData.InputBegan:Connect(function(inp)
			if inp.UserInputType == Enum.UserInputType.MouseButton1 then
				imageButton.MouseButton1Click:Fire()
			end
		end)
