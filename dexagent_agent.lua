...
local function initAfterMain(t) end
...
local function main()
	...
	ds.Init = function()
		local ok, err = pcall(function()
			...
			local labelPlayerData = Instance.new("TextLabel")
			labelPlayerData.Parent                 = scrollFrame
			...
			local function reconstruirLista()
				...
				if isListVisible then
					window:Resize(246, math.min(totalH, 400))
				else
					window:Resize(246, 24)
				end
			end
			...
			imageButton.MouseButton1Click:Connect(function()
				...
				if isListVisible then
					window:Resize(246, math.min(24 + (#playerButtons * 18) + 10, 400))
				else
					window:Resize(246, 24)
				end
			end)
		...
		if not ok then
			warn("DataStoreEditor Init error: " .. tostring(err))
		end
	end
